...
async function showBoard(turnCount) {
  ...
  const previousDisc = responseBody.previousDisc;
...
function warningMessage(previousDisc, nextDisc, winnerDisc) {
  if (nextDisc !== null){
    if (previousDisc === nextDisc){
      ...
      return `${discToStoring(skipped)}の番はスキップです`;
    }else{
      return null;
    } 
  } else {
    if (winnerDisc === WINNER_DRAW) {
      ...
      return `${discToStoring(winnerDisc)}の勝ちです`;
    }
  }
}
...
function showNextDiscMessage(nextDisc){
  if(nextDisc){
    nextDiscMessageElement.innerText = `次は${discToStoring(nextDisc)}の番です`
  } else {
